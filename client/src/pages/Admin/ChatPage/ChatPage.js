...
const ChatPage = () => {
    ...
    const handleSendClick = (e) => {
        e.preventDefault();
        setInput('')
        sendMessage(input, params.id, 'admin')
        .then(messages => {
            setChatItems(messages)
        })
                        ...
                        <div onClick={() => setContextMenuClass()} className='admin_chat_container'>
                            {
                                chatItems.map((chatItem, i) => 
                                    ...
                                    <div onContextMenu={(e) => handleContextMenu(e, chatItem.id)} id={`chat_message_${chatItem.id}`} className={`admin_chat_message_container ${chatItem.role == 'user'? 'user' : 'admin'} ${contextMenuClass==chatItem.id? 'open_menu' : ''}`}>
